...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a NewSVN address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no NewSVN: URI
...
{
    // Convert newsvn:// to newsvn:
    //
    //    Cannot handle this later, because newsvn:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open newsvn.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NewSVN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for NewSVN.lnk
...
{
    return GetAutostartDir() / "newsvn.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a newsvn.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=NewSVN\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the newsvn app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add newsvn app to startup item list
